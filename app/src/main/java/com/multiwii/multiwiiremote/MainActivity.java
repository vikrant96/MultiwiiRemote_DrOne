...
//            app.protocol.is_SET_RAW_RC_received = false;
	...
	public void onSensorsStateChangeRotate() {
		//Log.i(TAG,"Setting x and y of right stick");
			...
			if (!app.sensors.isRotateSensorSupported())
				...
				switch (inputMode) {
					case TOUCH:
						...
						Log.i(TAG,"Input mode switched to accelerometer");
					break;
					case ACCELEROMETER:
						inputMode = InputMode.TOUCH;
						dualJoystickView.stickR.returnHandleToCenter();
						Log.i(TAG,"Input mode switched to manual");
					break;
				}
			}
			break;
